...
\end{lispcode}
	...
	\end{enumerate}
	\textit{n.b.,} If a queue happens to be empty, then the consumer thread should handle it gracefully by briefly sleeping before polling again. The sleep is a small number, which you set to prevent busy waiting.\footnote{Busy waiting (or spinning) is when a consumer constantly asks an empty queue if it has any data in queue --- consequently, this tanks a systems performance.}
...
\end{lispcode}
	...
	The API Reference for \texttt{cl-freelock} is a dictionary of all global functions, involving unbounded Multi-Producer Multi-Consumer (MPMC), bounded MPMC, bounded Single Producer Single Consumer (SPSC), error handling, and atomic primitives.\footnote{\textit{n.b.,} The difference between atomic primitives and atomic operations is a matter concerning layers of abstraction---operations are composed of combinations of primitives and non-primitive procedures.}
	...
	\customitem{Description:}{Accessor for the value held within an \code{atomic-ref}---used internally by Compare and Swap (CAS) operations.\footnote{An accessor (\textit{i.e.,} A structure or slot accessor) is a procedure which admits access to an object's internal state.}\supcom\footnote{\textit{n.b.,} \code{atomic-ref-value} is internally a \code{defstruct}. In \textit{Common Lisp}, the use of \code{defstruct} acts as both a getter and setter by the implementation's compiler. A getter is a procedure which specifically returns a read-only value.}}
	\subsubsection{CAS}
	...
	\customitem{Example:}{}
\begin{lispcode}
(defparameter *work-queue* (fl:make-queue))
\end{lispcode}
	\subsubsection{QUEUE-PUSH}
	
	\customitem{Signature:}{\code{(queue-push queue object)}}
	
	\customitem{Description:}{Pushes an \code{object} into the unbounded queue.}
	
	\customitem{Returns:}{\code{T} upon a successful push.}
	\subsubsection{QUEUE-POP}
	
	\customitem{Signature:}{\code{(queue-pop queue)}}
	
	\customitem{Description:}{Pops an object from the unbounded queue.}
	
	\customitem{Returns:}{}
	\begin{enumerate}
		\item The \code{object} itself, or \code{NIL} if the queue is empty.
		\item A success boolean, \code{T} if an item was grabbed, \code{NIL} if the queue was empty.
	\end{enumerate}
	\textit{n.b.,} These two values must be captured with the \code{multiple-value-bind}.
	\subsubsection{QUEUE-EMPTY-P}
	
	\customitem{Signature:}{\code{(queue-empty-p queue)}}
	
	\customitem{Description:}{Checks if the unbounded queue is empty.}
	
	\customitem{Returns:}{\code{T} if empty, and \code{NIL} if not.}
	
	\textit{Warning}: In a highly concurrent environment, the state of a queue can change immediately after this call returns, so do not rely on it for flow control. If needed, it is best to rely on the success boolean returned by \code{queue-pop}.\footnote{This is called a Time-of-Check to Time-of-Use (TOCTOU) race condition. This is a race condition that specifically occurs when multiple threads or processes access shared data through concurrent means, but without adequate synchronization. \textit{n.b.,} TOCTOU race conditions are exploitable, and thus are security vulnerabilities. \cite{cwe367}}
